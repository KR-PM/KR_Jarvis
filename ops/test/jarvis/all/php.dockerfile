...
COPY ./ops/local/jarvis/all/php/conf.d/opcache.ini $PHP_INI_DIR/conf.d/
COPY ./ops/local/jarvis/all/supervisor_php/conf.d/ /etc/supervisor/conf.d/
COPY ./ops/local/jarvis/all/php.sh /
...
RUN echo "* * * * * root php /app/code/backend/jarvis/artisan schedule:run >> /dev/null 2>&1" >> /etc/crontab
RUN echo "\n \
...
RUN echo "\n \
alias fakedb='php artisan db:wipe && php artisan migrate:refresh --seed && php artisan db:seed --class=FakeSeeder'\n \
